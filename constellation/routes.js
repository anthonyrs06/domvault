...
//   #/read/<id>         → land ON the planet: full-screen reading view
//   #/timelapse         → the "watch it grow" replay
...
export function parseRoute(hash) {
  ...
  if (kind === 'timelapse') return { kind: 'timelapse' };
  if ((kind === 'galaxy' || kind === 'star' || kind === 'planet' || kind === 'read') && id) return { kind, id };
  return { kind: 'universe' };
}

export function formatRoute(r) {
  if (!r || r.kind === 'universe') return '#/';
  if (r.kind === 'timelapse') return '#/timelapse';
 ...
 * `#/read/<id>` resolves like `#/planet/<id>` plus `read: true` (land + open the page).
 * @returns {{view:{mode:string,galaxy?:object,star?:object}, planet:object|null, read?:boolean,
 *   timelapse?:boolean}|null} null = broken link
 */
export function resolveRoute(route, data, current) {
  const gs = (data && data.galaxies) || [];
  if (route.kind === 'timelapse') {
    return { view: { mode: 'universe', galaxy: null, star: null }, planet: null, timelapse: true };
  }
